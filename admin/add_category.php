...
<li class="breadcrumb-item active" style="font-size: 18px;font-weight: bold;color: black !important;">Manage Category</li>
</ol>
...
<input type='text' name="ctgy_neme" placeholder="Enter Category name" class="form-control"  style="border: 2px solid grey!important;" required>
...
$clt_dplc="select * from `category` where `name`='$categ_neme'";
$qst_clt_dplc=$db->query($clt_dplc);
$ctgy_count=mysqli_num_rows($qst_clt_dplc);

if($ctgy_count==0)
{
$ad_prdt="insert into `category` set
`name`='$categ_neme',
...
if($qst_ad_prdt)
{
echo "<script>window.alert('Category Added Successfully');window.location='';</script>";
...
$sno=1;
  ...
  $adm_neme = $clct_adm_dtl['name'];
    ...
    ?><tr>
        ...
        <td><?php if(permission::hidePemission('edit_category')){ ?><a href="edit_category.php?id=<?php echo $ctegy_id;?>&target=edit_category" class='btn btn-success'><i class='fa fa-edit'></i></a> <?php } ?> <?php if(permission::hidePemission('delete_category')){ ?><a href="delete_category.php?id=<?php echo $ctegy_id;?>" class='btn btn-danger'><i class='fa fa-trash'></i></a><?php } ?></td>
        </tr>
        <?php
    }
    ?>
