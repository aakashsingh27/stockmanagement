...
<li class="breadcrumb-item active" style="font-size: 18px;font-weight: bold;color: black !important;">View Role</li>
</ol>
    
<div class="col-md-12" >
    <?php  if(permission::hidePemission('add_role')){ ?>
    <a href="add_role.php?target=add_role" style='float:right' class='btn btn-primary'>Add Role</a>
    <?php 
    }
    ?>
    </div>
...
<th style="border-bottom:2px solid black !important;">Role name</th>
...
<th style="border-bottom:2px solid black !important;">Action</th>
   ...
   <tbody>     
<?php 
$usr_lst="select * from `roles_and_permission` ";
...
$sno=1;
    ...
    {
        $roleNmae=$clct_usr_lst['roles_name'];
        $detret=$clct_usr_lst['created_date_time'];
       $iid=$clct_usr_lst['id'];
    ...
    ?><tr>
        ...
        <td><?php echo $created_by_name['name'];?></td>
                <td><?php echo $detret;?></td>
        <td>
             <?php  if(permission::hidePemission('edit_role')){ ?>
            <a  href="edit_role.php?id=<?php echo $iid;?>&target=edit_role" class='btn btn-success'><i class='fa fa-edit'></i></a>
            <?php 
             }
            ?>
            <!--<a href="delete_user.php?id=<?php echo $ctegy_id;?>" onclick="return confirm('Are you sure want to delete this User?')"  class='btn btn-danger'><i class='fa fa-trash'></i></a>--></td>
        </tr>
        <?php
    }
    ?>
