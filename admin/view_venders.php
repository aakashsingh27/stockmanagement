...
<li class="breadcrumb-item active" style="font-size: 18px;font-weight: bold;color: black !important;">View Vendors</li>
</ol>
<?php  if(permission::hidePemission('add_vendor')){ ?>
   <div class="col-md-12" ><a href="add_vender.php?target=add_vendor" style='float:right' class='btn btn-primary'>Add Vendor</a></div>
<?php } ?>
...
<th style="border-bottom:2px solid black !important;">Company</th>
<th style="border-bottom:2px solid black !important;">Name</th>
<th style="border-bottom:2px solid black !important;">Phone</th>
...
$sno=1;
    ...
    {
        $company=$clct_usr_lst['company'];
        $name=$clct_usr_lst['name'];
        $phone=$clct_usr_lst['phone'];
         $email=$clct_usr_lst['email'];
    ...
    ?><tr>
        ...
        <td>
            <?php  if(permission::hidePemission('edit_vendor')){ ?>
            <a  href="edit_vender.php?id=<?php echo $ctegy_id;?>&target=edit_vendor" class='btn btn-success'><i class='fa fa-edit'></i></a>
            <?php } ?>
        
                    <?php  if(permission::hidePemission('delete_vendor')){ ?>
          <a href="delete_vendor.php?id=<?php echo $ctegy_id;?>&target=delete_vendor" onclick="return confirm('Are you sure want to delete this vendor?')"  class='btn btn-danger'><i class='fa fa-trash'></i></a>
          <?php } ?>
          </td>
        </tr>
        <?php
